...
static const yytype_int16 yyrline[] =
{
     ...
     154,   158,   170,   184,   193,   201,   205,   221,   222,   225,
     226,   227,   228,   229,   230,   231,   234,   240,   245,   253,
     260,   268,   275,   282,   290,   294,   298,   304,   307,   313,
     316,   318,   322,   326,   330,   334,   340,   344,   348,   352,
     356,   360,   364,   370,   374,   378,   384,   388,   392,   398,
     402,   406,   410,   416,   422,   430,   437
};
#endif
...
#line 206 "CminusParser.y"
                {
			//ADD CODE HERE
			//get value of int constant
			int symIndex = SymIndex(symtab,(yyvsp[-1].name));
			char *str = (char*) SymGetFieldByIndex(symtab, symIndex, SYM_NAME_FIELD);
			int val = atoi(str);

			//save size of int
			symIndex = SymIndex(symtab,(yyvsp[-3].name));
			SymPutFieldByIndex(symtab,symIndex, SYMTAB_SIZE_FIELD,(Generic)(4*val));
...
#line 241 "CminusParser.y"
                {
			//ADD CODE HERE
			emitBranchEnd(instList, symtab, (yyvsp[-2].symIndex));
		}
...
#line 246 "CminusParser.y"
                {
			//ADD CODE HERE
			emitBranchEnd(instList, symtab, (yyvsp[0].symIndex));
		}
...
#line 254 "CminusParser.y"
                {
			//ADD CODE HERE
			(yyval.symIndex) = emitThen(instList, symtab, (yyvsp[-1].symIndex));
		}
...
#line 261 "CminusParser.y"
                {
			//ADD CODE HERE
			(yyval.symIndex) = emitTest(instList, symtab, (yyvsp[-1].symIndex));
		}
...
#line 269 "CminusParser.y"
                {
			//ADD CODE HERE
			emitLoopBranch(instList,symtab,(yyvsp[-2].symIndex),(yyvsp[-1].symIndex));
		}
...
#line 276 "CminusParser.y"
                {
			//ADD CODE HERE
			(yyval.symIndex) = emitLoopTest(instList,symtab,(yyvsp[-1].symIndex));
		}
...
#line 283 "CminusParser.y"
                {
			//ADD CODE HERE
			(yyval.symIndex) = emitLoopEnd(instList,symtab);
		}
...
#line 417 "CminusParser.y"
			...
			//int offset = (int)SymGetFieldByIndex(symtab,symIndex,SYMTAB_OFFSET_FIELD); CODE REMOVED, DID NOTHING
		}
