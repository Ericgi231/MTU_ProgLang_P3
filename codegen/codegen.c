...
void addIdToSymtab(DNode node, AddIdStructPtr data) {
	...
	data->offset += (int)SymGetFieldByIndex(data->symtab, symIndex, SYMTAB_SIZE_FIELD); //ADDED---------------------------------------------
}

//New Code
// got lazy and did not add function comments, just included code comments

int emitComputeVariableAddressFromArray(DList instList, SymTable symtab, int varIndex, int arrayOffset)
{
	//get a whole bunch of reg info
    int regIndex = getFreeIntegerRegisterIndex(symtab);
    char* regName = (char*)SymGetFieldByIndex(symtab, regIndex, SYM_NAME_FIELD);
    char* regArrOffName = (char*)SymGetFieldByIndex(symtab, arrayOffset, SYM_NAME_FIELD);
    int offset = (int)SymGetFieldByIndex(symtab, varIndex, SYMTAB_OFFSET_FIELD);
    char offsetStr[10];
    snprintf(offsetStr, 9, "%d", offset);

	//generate instructions for indexing array, add to instruction list
    char* inst;
    if (offset < 0) {
        inst = nssave(4, "\tadd ", regName, ", $fp, ", offsetStr);
	} else {
    ...
    dlinkAppend(instList, dlinkNodeAlloc(inst));
    inst = nssave(6, "\tmul ", regMultName, ", ", regMultName, ", ", regArrOffName);
    dlinkAppend(instList, dlinkNodeAlloc(inst));
    inst = nssave(6, "\tadd ", regName, ",", regName, ", ", regMultName);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

	//free old regs
	int reg = SymGetFieldByIndex(symtab, regMultIndex, SYMTAB_REGISTER_INDEX_FIELD);
    freeIntegerRegister(reg);
	reg = SymGetFieldByIndex(symtab, arrayOffset, SYMTAB_REGISTER_INDEX_FIELD);
    freeIntegerRegister(reg);

    return regIndex;
}

void emitBranchEnd(DList instList, SymTable symtab, int endLabelIndex)
{
	//find label name and add empty label to instruction list
	char *label = SymGetFieldByIndex(symtab, endLabelIndex, SYM_NAME_FIELD);
    char *inst = nssave(2, label, ":\tnop");
    dlinkAppend(instList, dlinkNodeAlloc(inst));
}

static void createLabel(char label[8])
{
	//create new label, increment label count
    snprintf(label, 19, ".L%d", labelCount++);
}

int emitTest(DList instList, SymTable symtab, int regIndex)
{
	//get new label
    char label[8];
    createLabel(label);

	//add branch on equality to instruction list	
    char* regName = SymGetFieldByIndex(symtab, regIndex, SYM_NAME_FIELD);
    char *inst = nssave(4, "\tbeq ", regName, ", $0, ", label);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

	//free old reg
	//int reg = SymGetFieldByIndex(symtab, regIndex, SYMTAB_REGISTER_INDEX_FIELD);
    //freeIntegerRegister(reg);
    return SymIndex(symtab, label);
}

int emitThen(DList instList, SymTable symtab, int elseLabelIndex)
{
	//get new label
    char label[8];
    createLabel(label);

	//add jump to label to instruction list
    char* inst = nssave(2, "\tj ", label);
    dlinkAppend(instList, dlinkNodeAlloc(inst));
    emitBranchEnd(instList, symtab, elseLabelIndex);
    return SymIndex(symtab, label);
}

void emitLoopBranch(DList instList, SymTable symtab, int beginLabelIndex, int endLabelIndex)
{
	//find label for start of loop and jump to it, add to instruction list
	char *label = SymGetFieldByIndex(symtab, beginLabelIndex, SYM_NAME_FIELD);
    char* inst = nssave(2, "\tj ", label);
    dlinkAppend(instList, dlinkNodeAlloc(inst));

	//create empty label for end of loop, add to instruction list
	char *end = SymGetFieldByIndex(symtab, endLabelIndex, SYM_NAME_FIELD);
    inst = nssave(2, end, ":\t nop");
    dlinkAppend(instList, dlinkNodeAlloc(inst));
}

int emitLoopTest(DList instList, SymTable symtab, int regIndex)
{
	//get new label
    char label[8];
    createLabel(label);

	//create logic for while loop test, add to instruction list
...
int emitLoopEnd(DList instList, SymTable symtab)
{
	//get new label
    char label[8];
    createLabel(label);

	//create end label for loop and add to instruction list
    char* inst = nssave(2, label, ":\tnop");
    dlinkAppend(instList, dlinkNodeAlloc(inst));
